...
class CoinCapPriceAsyncStreamService: NSObject {
    
    private let session = URLSession(configuration: .default)
    private var wsTask: URLSessionWebSocketTask?
    private var pingTryCount = 0
    
    private let coinDictionaryCurrentValueStream = CurrentValueAsyncStream(initialValue: [String: Coin]())
    var coinDictionaryStream: AsyncStream<[String: Coin]> { coinDictionaryCurrentValueStream.stream }
...
extension CoinCapPriceAsyncStreamService: URLSessionWebSocketDelegate {
